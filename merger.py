...
def compute_merging(clouds):
    if(len(clouds) == 2):
        if(cloud = merge_two_las(clouds[0], clouds[1])):
            clouds = []
            clouds.append(cloud)
...
def ask_for_merging(name_1, name_2):
    while True:
        try:
            resp = str(input("[\033[1;34m#\033[0m] Merger - Do you want to merge \033[1;32m%s\033[0m and \033[1;32m%s\033[0m ? "% (name_1, name_2)))
            if(resp == "true" or resp == "True" or resp == "y" or resp == "Y" or resp == "yes"):
                return True
            if(resp == "false" or resp == "False" or resp == "n" or resp == "N" or resp == "no"):
                return False
        except ValueError:
            print("[\033[1;31merror\033[0m] Merger - Sorry, I didn't understand that.")
            continue
...
def merge_two_las(cloud_1, cloud_2):
    if(ask_for_merging(name_1, name_2)):
        print("[\033[1;32mok\033[0m] Merging \033[1;32m%s\033[0m and \033[1;32m%s\033[0m"% (cloud_1["name"], cloud_2["name"]))
